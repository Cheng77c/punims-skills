...
**数据与接线铁律:**
- **decoy 必须由 `philosopher-database` 步构建**:把 target-only FASTA 直接喂给 MSFragger 会产生零 decoy 序列,FDR 估计静默崩溃。DAG 必须包含该步;
...
- 数据在**项目共享盘 / 个人盘**:**谱图**用「共享盘/个人盘转数据集」能力(bohrium-dataset-manager-test 的「从共享盘/个人盘建数据集」)**直接转成 dataset(无需下载)**,拿到 `/bohr/<名>/v1` 填入 raw_files[];**FASTA 不转 dataset,只下载它**进任务目录走 `-p`(见第 4 步)。
- 已是 **dataset**(自建或网页端上传):`bohr dataset list -p <项目>` 找到,把 `/bohr/<名>/v1/<文件>` 填入 raw_files[];**不知内部文件名/路径时,用 bohrium-dataset-manager-test 的「列出数据集内文件」(`dataset_manager.py files --id <ID>`)拿确切路径——别猜、也别反问用户**。
...
```json
{
  ...
  "edges": [
    {"src": "db", "dst": "mf"}, {"src": "mf", "dst": "pp"}, {"src": "pp", "dst": "rp"}
  ],
...
> 语义接线 —— 例如 **ptmshepherd 需要 mzML**、open 搜索需 crystalc、ionquant 需谱图+psm.tsv。
...
`peptideprophet` · `ptmprophet` · `philosopher-report` · `freequant` · `labelquant` · `abacus` ·
`ionquant` · `ptmshepherd` ·
...
| Philosopher LFQ | `philosopher-report → freequant` | `freequant` 更新 PSM/肽/离子定量表 |
| Philosopher 标记定量 | `philosopher-report → labelquant` | 需 brand/plex/annotation_file |
| 跨实验汇总 | 多个 report/proteinprophet/iprophet → `abacus` | `abacus` 至少需要两个实验输入 |
...
| 数据在哪 | 怎么进作业 | 作业内可写 |
|---|---|---|
| **谱图(`.raw`/`.mzML`,任意大小)——默认建 dataset** | 工作区本地:`make_dataset.py`;共享盘/个人盘:「转数据集」能力(bohrium-dataset-manager-test,**直接转、无需下载**)。拿 `/bohr/<名>/v1/<文件>` 填 raw_files[] | ❌ 只读 |
...
- **philosopher-database 必须存在**:没有 target+decoy .fas,MSFragger 无法估 FDR;其排序与注入由 v4 plan 显式表达。
