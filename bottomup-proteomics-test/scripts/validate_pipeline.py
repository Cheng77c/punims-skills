...
_CONNECTIONS = _load("connection_rules.json")
...
def validate_config(cfg: dict) -> list[str]:
    ...
    for e in cfg.get("edges") or []:
        ...
        source_tool = tool_by_step.get(e.get("src"))
        target_tool = tool_by_step.get(e.get("dst"))
        bu_tools = set(_CONNECTIONS.get("bottom_up_tools") or [])
        if (
            source_tool in bu_tools
            and target_tool in bu_tools
            and target_tool
            not in _CONNECTIONS.get("rules", {}).get(source_tool, [])
        ):
            errs.append(
                f"不允许的连线:{e.get('src')}({source_tool}) → "
        ...
        parent_count = len(
            {
                ...
                if edge.get("dst") == step.get("step_id")
            }
        )
        if parent_count < 2:
            errs.append(
