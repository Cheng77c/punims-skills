...
| `Dataset ... has been deleted` | 根因=Bohrium 给名字加随机后缀(`/bohr/<name>-xxxx/v1`),别假设 `/bohr/<name>/v1`;**make_dataset.py 已自动从 API 返回真实路径**,用它给的 `spectrum_mount`/`--dataset-path` 即可。create 后 `dataset_manager.py list --project-id <pid>` 可查真实 path(不要手写 bohr dataset list / curl) |
