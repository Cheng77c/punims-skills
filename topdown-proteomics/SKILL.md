...
3. **谱图默认一律走 dataset(不论大小)**:共享盘/个人盘的谱图**直接转 dataset、无需下载**,工作区本地用 `make_dataset.py`;仅当用户**主动要求**"直接上传"且谱图 ≤100MB 才 `-p`。**建集失败不是启用 `-p` 的理由**——已经翻过车:sandbox 建集报错后,agent 自行把 92MB 谱图下载到工作区再建集,而报错里白纸黑字写着"不要下载到工作区绕过"。建集失败就停下报错,别自己找替代路线。**唯一需要下载的是 FASTA**(需可写)。结果用 `collect_results.py` 取。
...
### 用户问「我该怎么准备数据?」——回答的优先级(别自由发挥)

这是纯咨询,**不提交任何作业**。按这个顺序讲,别把顺序讲反了:

1. **首选:数据已经在共享盘/个人盘上 → 服务端直接转成数据集,免下载中转。**
   这是最快的路,几个 GB 的谱图也不用经过工作区。让用户报出盘和路径即可。
   **已经翻过车:agent 把「网页端上传」讲成首选、盘上直转讲成备选 —— 讲反了。**
2. 备选:数据在用户本地电脑 → 网页端上传成数据集,或传到盘上再走第 1 条。
3. **主动告诉用户:同一份数据传过一次就不必再传** —— 建集前会按文件内容(名+大小)
   自动查重,命中就零传输复用。**这是真实能力,用户最关心「我是不是又要传一遍几十 GB」,
   一定要讲。**(已经翻过车:agent 答数据准备时完全没提查重,白白丢掉一个卖点。)
4. **FASTA 不做数据集**,随作业打包上传即可(几十 MB 都没问题)——因为搜索引擎要在
   FASTA 同目录写索引,只读挂载会失败。别对用户说「要小于多少 MB 才能直传」这类假门槛。
5. 谱图格式:`.raw` / `.mzML` / Bruker timsTOF `.d` 都支持。**不要武断说只支持前两种。**
6. 最后追问:做 **Top-down 还是 Bottom-up**、DDA 还是 DIA、是否要定量。
