...
_PARAM_SCHEMA_PATH = Path(__file__).resolve().parent / "param_schema.json"
_PARAM_SCHEMA = json.loads(_PARAM_SCHEMA_PATH.read_text()) if _PARAM_SCHEMA_PATH.exists() else {}
...
def _check_schema(i, tool, params, errors):
    schema = _PARAM_SCHEMA.get(tool, {})
    for name, value in params.items():
        definition = schema.get(name)
        if definition is None:
            suggestions = difflib.get_close_matches(
                name, list(schema), n=2, cutoff=0.4
            )
            fix = (
                f"改为 {' / '.join(suggestions)}"
                if suggestions
                else "只用 references/parameters.md 中列出的参数名"
            )
            errors.append(_err(i, tool, name, "未知参数名", fix=fix))
            continue
        ...
        if not _type_ok(value, expected):
            errors.append(
                _err(
                    ...
                    tool,
                    name,
                    f"类型应为 {expected},实际 {type(value).__name__}",
                    fix=f"按 {expected} 类型填写",
                )
            )
            continue
        ...
        if isinstance(value, (int, float)) and not isinstance(value, bool):
            ...
            if minimum is not None:
                invalid = value <= minimum if definition.get("xmin") else value < minimum
                if invalid:
                    operator = ">" if definition.get("xmin") else "≥"
                    errors.append(
                        _err(i, tool, name, f"必须 {operator} {minimum}")
                    )
            if maximum is not None and value > maximum:
                errors.append(_err(i, tool, name, f"必须 ≤ {maximum}"))
    for name, definition in schema.items():
        if definition.get("required") and name not in params:
            errors.append(_err(i, tool, name, "缺必填参数"))
...
def validate(cfg: dict) -> dict:
    ...
    for i, s in enumerate(steps):
        ...
        if tool not in TOOLS:
            ...
            continue
        _check_schema(i, tool, dict(s.get("params") or {}), errors)
