...
- **只有一个镜像**,地址的**单一源 = skill 根的 `image.txt`**(版本迭代只改这一个文件;脚本都从它读,env `IMAGE_ADDRESS` 可临时覆盖)。
  ...
  执行器入口 `/opt/topdown/run.sh`。skill 保留简单的 `inputs + steps` 写法,提交时编译成
  `contract_version: 4` 的 `execution_plan.json`;镜像只按显式 bindings/edges 执行。
...
- 数据在**项目共享盘 / 个人盘**:**谱图**用「共享盘/个人盘转数据集」能力(bohrium-dataset-manager-test 的「从共享盘/个人盘建数据集」)**直接转成 dataset(无需下载)**,拿到 `/bohr/<名>/v1` 填入 pipeline 的 inputs;**FASTA 不转 dataset,只下载它**进任务目录走 `-p`(见第 4 步)。
- 已是 **dataset**(自建或网页端上传):`bohr dataset list -p <项目>` 找到,把 `/bohr/<名>/v1/<文件>` 填入 inputs;**不知内部文件名/路径时,用 bohrium-dataset-manager-test 的「列出数据集内文件」(`dataset_manager.py files --id <ID>`)拿确切路径——别猜、也别反问用户**。
...
按下方 schema 写一个 `pipeline.json`,放进任务目录。`steps` 是线性步骤列表,
...
| 数据在哪 | 怎么进作业 | 作业内可写 |
|---|---|---|
| **主输入谱图(`.raw`/`.mzML`/`.msalign`/`.pbf`,任意大小)——默认建 dataset** | 工作区本地:`make_dataset.py`;共享盘/个人盘:「转数据集」能力(bohrium-dataset-manager-test,**直接转、无需下载**)→ `--dataset-path` 挂载。拿 `/bohr/<名>/v1/<文件>` 填 inputs | ❌ 只读 |
...
`submit_pipeline.py` 自动把 `inputs` 中的本地路径放入任务目录、生成 `execution_plan.json`;
`/bohr/…` 路径作为 dataset 挂载引用,原样保留。任务目录是自包含的,不会在提交后删除。
...
```
脚本将 **pipeline.json + execution_plan.json + 本地输入**置入 `-p` 上传目录,以配置的镜像提交;
执行器位于镜像内,不随包上传。
...
- `steps` 按序列出工具;`submit_pipeline.py` 在 skill 侧把每步输入编译为显式 binding,
  镜像不再根据工具名推断接线。你仍只需列工具+参数。
